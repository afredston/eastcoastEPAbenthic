...
#E) SAMPYEAR 
#F) WORMS_APHIAID 
BC2000cor<-BC2000cor %>% select(SITE_ID, tot_num_m2, LON_DD, LAT_DD, SAMPYEAR, WORMS_APHIAID)
...
BC2010a<-BC2010a %>% mutate(SITE_ID = SITE_ID.x,
...
BC2015<-BC2015 %>% mutate(tot_num_m2 = TOTAL/as.numeric(FINAL_GRAB_AREA),
                   SAMPYEAR = YEAR,
...
BC2020<-BC2020 %>% mutate(tot_num_m2 = DENSITY,
                   SAMPYEAR = 2020, 
                   WORMS_APHIAID = APHIA_ID) %>%
  select(SITE_ID, tot_num_m2, LON_DD, LAT_DD, SAMPYEAR, WORMS_APHIAID)

all.data<-rbind(BC2000cor, BC2010a, BC2015, BC2020)
all.data
```
